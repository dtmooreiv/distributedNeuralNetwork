...
Based on the code in network2.py

Used for remote procedure calls from network_master. Trains one epoch
on the slave's portion of the training data by being given the
network_master's most up-to-date parameters, and then returns the updates
to these parameters for the master to use. 

Think of slaves as servers that respond to SGD-esque requests.
...
class Network(object):
    ...
    def run_mini_batch(self, mini_batch, eta, lmbda, n):
        """Update the network's weights and biases by applying gradient
        descent using backpropagation to a single mini batch.  The
        ``mini_batch`` is a list of tuples ``(x, y)``, ``eta`` is the
        learning rate, ``lmbda`` is the regularization parameter, and
    ...
    def save(self, filename):
        ...
        f = open(filename, "w")
        json.dump(self.stringify(), f)
        f.close()

    def stringify(self):
        """Dump the neural network to a string"""
...
def loads_net(string):
    """Load a neural network from a string. Returns an instance of Network
    """
...
class NetworkRPCServer(symmetricjsonrpc.RPCServer):
...
if __name__ == '__main__':
    # Load in data
    training_data, validation_data, test_data = mnist_loader.load_data_wrapper('/neuralNet/data/mnist.pkl.gz')
    print('Accepting connection')
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    s.bind(('',4712))
    s.listen(1)
    server = NetworkRPCServer(s, name='NetworkSlave')
    server.join()